...
def get_treasury_data(years=2):
    ...
    print(f"📅 {end_date.strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 50)
    print("Fetching Treasury rates from Yahoo Finance...")
...
def analyze_rates(df, n_days=90):
    ...
    order = ['13W', '5Y', '10Y', '30Y']
    for maturity in order:
        if maturity in current.index:
            rate = current[maturity]
            print(f"{maturity:>3}: {rate:6.3f}%")
    
    inversion_status = "N/A"
    spread = None
    if '13W' in current.index and '10Y' in current.index:
        spread = current['10Y'] - current['13W']
        print(f"\n📈 KEY SPREADS")
        print("-" * 20)
        print(f"13W-10Y: {spread:+.3f}% ", end="")
        if spread < 0:
            print("(🔴 INVERTED)")
            inversion_status = "INVERTED"
        elif spread < 0.5:
...
def plot_curve(df, stats, n_days=90):
    ...
    recent_data = df.tail(252)
    colors = ['red', 'green', 'blue', 'orange']
    for i, rate in enumerate(['13W', '5Y', '10Y', '30Y']):
        if rate in df.columns:
            fig.add_trace(
                go.Scatter(
                    x=recent_data.index,
    ...
    if '10Y' in df.columns and '13W' in df.columns:
        spread_13w10y = (df['10Y'] - df['13W']).tail(252)
        fig.add_trace(
            go.Scatter(
                x=spread_13w10y.index,
                y=spread_13w10y,
                mode='lines',
                line=dict(width=2, color='red'),
                fill='tonexty',
    ...
    fig.update_layout(
        title=dict(text=f'US Treasury Analysis (Yahoo Finance) - {today_str}', x=0.5),
        height=800,
        showlegend=False
    )
    ...
    fig.update_yaxes(title_text="Yield (%)", row=1, col=2)
    fig.update_xaxes(title_text="Date", row=2, col=1)
    fig.update_yaxes(title_text="10Y-13W Spread (%)", row=2, col=1)
    fig.update_xaxes(title_text="Maturity", row=2, col=2)
    fig.update_yaxes(title_text="Yield (%)", row=2, col=2)
...
def main():
    ...
    print("✅ DAILY TREASURY ANALYSIS COMPLETE")
    print("📁 Files generated:")
    print("    - treasury_analysis_plotly.html")
    print("    - treasury_analysis.png")
    print("    - treasury_data_YYYYMMDD.csv")
    print("    - treasury_summary_YYYYMMDD.csv")
